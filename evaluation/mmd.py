...
def emd(x, y, distance_scaling=1.0):
    ...
    emd_value = ot.emd2(x, y, distance_mat)
...
def compute_mmd(samples1, samples2, kernel, is_hist=True, cached_d1_path=None, *args, **kwargs):
    ...
    if cached_d1_path is not None and os.path.exists(cached_d1_path):
        d1 = np.load(cached_d1_path)
    else:
        d1 = disc(samples1, samples1, kernel, *args, **kwargs)
        np.save(cached_d1_path, d1)
    d2 = disc(samples2, samples2, kernel, *args, **kwargs)
    d3 = disc(samples1, samples2, kernel, *args, **kwargs)
    return d1 + d2 - 2 * d3

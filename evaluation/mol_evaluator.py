...
_base_dir = os.path.join(os.path.dirname(__file__), 'filter_moses')
...
def average_agg_tanimoto(stock_vecs, gen_vecs,
    ...
    #stock_vecs = stock_vecs.astype(np.float32)
    for j in range(0, stock_vecs.shape[0], batch_size):
        x_stock = torch.tensor(stock_vecs[j:j + batch_size], dtype=torch.float).to(device).float()
        for i in range(0, gen_vecs.shape[0], batch_size):
            y_gen = torch.tensor(gen_vecs[i:i + batch_size], dtype=torch.float).to(device).float()
...
def cos_similarity(ref_counts, gen_counts, eps=1e-12):
    """Numerically safe cosine similarity for very large count vectors.

    Normalizes each vector independently before dotting to avoid SciPy-side overflow
    or sqrt-of-negative issues.
    """
    if len(ref_counts) == 0 or len(gen_counts) == 0:
        return np.nan
    ...
    if ref_norm < eps or gen_norm < eps:
        # Cosine is undefined when either vector is near-zero.
        return np.nan

    ref_vec /= ref_norm
    gen_vec /= gen_norm

    # Dot of unit vectors = cosine similarity = 1 - cosine_distance.
...
def get_all_metrics(gen, test=None, train=None, index=None,
                    n_jobs=32, device='cpu', batch_size=512, pool=None):
    """
    support index: 'FCD', 'Novelty', 'Uniqueness', 'SNN', 'Frag', 'Scaf', 'logP', 'SA', 'QED', 'weight', 'Filters'
    """
    global _ptest_cache

    # all_index = ['FCD', 'Novelty', 'Uniqueness', 'SNN', 'Frag', 'Scaf', 'logP', 'SA', 'QED', 'weight', 'Filters', 'NSPDK']
    all_index = ['fcd', 'novelty', 'uniqueness', 'snn', 'frag', 'scaf', 'logp', 'sa', 'qed', 'weight', 'filters', 'NSPDK']
    if index is None:
        index = ['fcd', 'novelty', 'uniqueness']
    if isinstance(index, str):
        index = [index]
    index = [i.lower() for i in index]  # Normalize to lowercase for comparison.
    ...
    gen_smiles = remove_invalid(gen, canonize=True)

    metrics = {}
    if 'uniqueness' in index:
        k = len(gen_smiles)
        metrics[f'Uniqueness'] = fraction_unique(gen_smiles, k, pool)


    if _ptest_cache is None:
        # The test set is fixed per run, so cache intermediate statistics.
        ptest_cache_path = os.path.join(PathManager.DATA_CACHE_DIR, f'ptest-{PathManager.HOSTNAME}.pkl')
        if os.path.exists(ptest_cache_path):
            _ptest_cache = pickle.load(open(ptest_cache_path, 'rb'))
        else:
            logger.info(f"Compute ptest cache to {ptest_cache_path}")
            _ptest_cache = compute_intermediate_statistics(test, n_jobs=n_jobs,
            ...
            logger.info(f"Save ptest cache to {ptest_cache_path}")

   
    if 'fcd' in index:
        kwargs_fcd = {'n_jobs': n_jobs, 'device': device, 'batch_size': batch_size}
        metrics['FCD'] = FCDMetric(**kwargs_fcd)(gen=gen_smiles, pref=_ptest_cache['FCD'])


    

    gen_rdkit_mols = mapper(pool)(get_mol, gen_smiles)
    
    kwargs = {'n_jobs': pool, 'device': device, 'batch_size': batch_size}
    if 'snn' in index:
        metrics['SNN'] = SNNMetric(**kwargs)(gen=gen_rdkit_mols, pref=_ptest_cache['SNN'])
    if 'frag' in index:
        metrics['Frag'] = FragMetric(**kwargs)(gen=gen_rdkit_mols, pref=_ptest_cache['Frag'])
    if 'scaf' in index:
        metrics['Scaf'] = ScafMetric(**kwargs)(gen=gen_rdkit_mols, pref=_ptest_cache['Scaf'])
    if 'filters' in index:
        metrics['Filters'] = fraction_passes_filters(gen_rdkit_mols, pool)
    if 'nspdk' in index:
        # NSPDK is expensive to compute.
        metrics['NSPDK'] = nspdk_eval_fn(gen_rdkit_mols, test)

    # Properties
    for name, func in [('logP', logP), ('SA', SA), ('QED', QED), ('weight', weight)]:
        if name.lower() not in index: continue
        metrics[name] = WassersteinMetric(func, **kwargs)(gen=gen_rdkit_mols, pref=_ptest_cache[name])

    if train is not None and 'novelty' in index:
        metrics['Novelty'] = novelty(gen_rdkit_mols, train, pool)

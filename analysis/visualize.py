...
def draw_graph_list(graph_list, row, col, f_path, iterations=100, layout='spring', is_single=False, k=1,
    ...
    if isinstance(graph_list[0], nx.Graph):
        G_list = graph_list
    else:
...
def visualize_mols(mol_list, row, col, fig_path):
    n_mol = row * col
    img = Draw.MolsToGridImage(mol_list[:n_mol], subImgSize=(400, 400), molsPerRow=col)
    img.save(fig_path)
